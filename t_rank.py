class TRank(object):
    ...
    def __init__(self, bwt, characters, cp_interval=4):
        ...
        # setting all values to negative 1, because t-rank should be 0 at first occurence
    ...
    def create_checkpoints(self):
        ...
        for i, c in enumerate(self.bwt):
            self.char_occurences[c] += 1  # track occurrence of letter
    ...
    def rank_at_row(self, char, row):
        """returns number of character occurrences up to given row.
        INCLUDING occurrences at given row"""
        ...
        # previous cp_index + 1 because the checkpoints count occurrences that happen on their index
        occurrences_after_checkpoint = self.count_up(char, prev_cp_index + 1, row + 1)
        # print(prev_cp_index, row
        return t_rank_at_prev_index + occurrences_after_checkpoint

    def count_up(self, char, from_index, to_index):
        """counts occurrences of char between from_index and to_index"""
        occurrences = 0
